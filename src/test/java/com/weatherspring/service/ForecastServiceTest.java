...
class ForecastServiceTest {
    ...
    @Mock
    private ForecastRecordRepository forecastRecordRepository;
    ...
    void setUp() {
        ...
        forecastRecord = ForecastRecord.builder()
                .id(1L)
    ...
    void getForecast_WithSaving_SavesForecastRecords() {
        ...
        when(weatherApiClient.getForecast("London", 3)).thenReturn(apiResponse);
        when(forecastMapper.toDtoFromApi(apiResponse, "London")).thenReturn(forecasts);
        when(locationService.findOrCreateLocation(anyString(), anyString(), anyDouble(), anyDouble(), any()))
    ...
    @Test
    void getForecast_WithInvalidDays_ThrowsException() {
        // Act & Assert
        assertThatThrownBy(() -> forecastService.getForecast("London", 0, false))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("between 1 and 14");

        assertThatThrownBy(() -> forecastService.getForecast("London", 15, false))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("between 1 and 14");
    }
    ...
    void getFutureForecasts_ReturnsOnlyFutureForecasts() {
        ...
        assertThat(result).isNotNull();
        assertThat(result).hasSize(1);
        assertThat(result.get(0).getForecastDate()).isAfter(LocalDate.now());
    }
