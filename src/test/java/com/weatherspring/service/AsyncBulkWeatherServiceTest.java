 ...
 *
 * <p>Tests async bulk operations with CompletableFuture handling,
 * partial failures, and virtual thread execution.</p>
 */
...
class AsyncBulkWeatherServiceTest {
    ...
    void setUp() {
        ...
        asyncBulkWeatherService = new AsyncBulkWeatherService(
                weatherService, forecastService, locationService, meterRegistry, 300);
    }

    @Test
    void fetchBulkWeatherAsync_WithValidLocations_ReturnsWeatherData() throws ExecutionException, InterruptedException {
        // Arrange
        List<String> locations = Arrays.asList("London", "Paris", "Tokyo");
        when(weatherService.getCurrentWeather(anyString(), anyBoolean()))
            .thenReturn(weatherDto);

        // Act
        CompletableFuture<List<WeatherDto>> future = asyncBulkWeatherService
            .fetchBulkWeatherAsync(locations, false);
    ...
    @Test
    void fetchBulkWeatherAsync_WithPartialFailures_ReturnsSuccessfulResults() throws ExecutionException, InterruptedException {
        // Arrange
        List<String> locations = Arrays.asList("London", "InvalidLocation", "Paris");

        when(weatherService.getCurrentWeather("London", false))
            .thenReturn(weatherDto);
        when(weatherService.getCurrentWeather("InvalidLocation", false))
            .thenThrow(new RuntimeException("Location not found"));
        when(weatherService.getCurrentWeather("Paris", false))
            .thenReturn(weatherDto);
    ...
    @Test
    void fetchBulkWeatherAsync_WithSaveToDatabase_SavesData() throws ExecutionException, InterruptedException {
        // Arrange
        List<String> locations = Arrays.asList("London");
        when(weatherService.getCurrentWeather(anyString(), eq(true)))
            .thenReturn(weatherDto);

        // Act
        CompletableFuture<List<WeatherDto>> future = asyncBulkWeatherService
            .fetchBulkWeatherAsync(locations, true);
        future.get();
    ...
    @Test
    void fetchBulkWeatherAsync_WithEmptyList_ReturnsEmptyResult() throws ExecutionException, InterruptedException {
        // Arrange
        List<String> locations = Collections.emptyList();

        // Act
        CompletableFuture<List<WeatherDto>> result = asyncBulkWeatherService.fetchBulkWeatherAsync(locations, false);
    ...
    @Test
    void fetchBulkForecastAsync_WithValidLocations_ReturnsForecastData() throws ExecutionException, InterruptedException {
        // Arrange
        List<String> locations = Arrays.asList("London", "Paris");
        List<ForecastDto> forecastList = Arrays.asList(forecastDto);

        when(forecastService.getForecast(anyString(), eq(3), anyBoolean()))
            .thenReturn(forecastList);

        // Act
        CompletableFuture<List<List<ForecastDto>>> future = asyncBulkWeatherService
            .fetchBulkForecastAsync(locations, 3, false);
    ...
    @Test
    void fetchBulkForecastAsync_WithPartialFailures_ReturnsSuccessfulResults() throws ExecutionException, InterruptedException {
        // Arrange
        List<String> locations = Arrays.asList("London", "InvalidLocation");
        List<ForecastDto> forecastList = Arrays.asList(forecastDto);

        when(forecastService.getForecast("London", 7, false))
            .thenReturn(forecastList);
        ...
        // Act
        CompletableFuture<List<List<ForecastDto>>> future = asyncBulkWeatherService
            .fetchBulkForecastAsync(locations, 7, false);
        List<List<ForecastDto>> results = future.get();
    ...
    @Test
    void bulkUpdateWeatherAsync_WithValidLocationIds_ReturnsSuccessCount() throws ExecutionException, InterruptedException {
        // Arrange
        ...
        // Act
        CompletableFuture<Integer> future = asyncBulkWeatherService
    ...
    @Test
    void bulkUpdateWeatherAsync_WithPartialFailures_ReturnsPartialSuccessCount() throws ExecutionException, InterruptedException {
        // Arrange
        List<Long> locationIds = Arrays.asList(1L, 2L, 3L);

        when(weatherService.getCurrentWeatherByLocationId(1L, true))
            .thenReturn(weatherDto);
        when(weatherService.getCurrentWeatherByLocationId(2L, true))
            .thenThrow(new RuntimeException("Location not found"));
        when(weatherService.getCurrentWeatherByLocationId(3L, true))
            .thenReturn(weatherDto);
    ...
    @Test
    void bulkRefreshForecastsAsync_WithValidLocationIds_ReturnsSuccessCount() throws ExecutionException, InterruptedException {
        // Arrange
        ...
        // Act
        CompletableFuture<Integer> future = asyncBulkWeatherService
            .bulkRefreshForecastsAsync(locationIds, 7, true);
        Integer successCount = future.get();
    ...
    @Test
    void bulkRefreshForecastsAsync_WithPartialFailures_ReturnsPartialSuccessCount() throws ExecutionException, InterruptedException {
        // Arrange
        List<Long> locationIds = Arrays.asList(1L, 2L, 3L);
        List<ForecastDto> forecastList = Arrays.asList(forecastDto);

        when(forecastService.getForecastByLocationId(1L, 14, true))
            .thenReturn(forecastList);
        when(forecastService.getForecastByLocationId(2L, 14, true))
            .thenThrow(new RuntimeException("API error"));
        when(forecastService.getForecastByLocationId(3L, 14, true))
            .thenReturn(forecastList);

        // Act
        CompletableFuture<Integer> future = asyncBulkWeatherService
            .bulkRefreshForecastsAsync(locationIds, 14, true);
        Integer successCount = future.get();
    ...
    @Test
    void refreshAllLocationsAsync_WithNoLocations_ReturnsZeroResult() throws ExecutionException, InterruptedException {
        // Arrange
        when(locationService.getAllLocations()).thenReturn(Collections.emptyList());

        // Act
        CompletableFuture<BulkOperationResult> future = asyncBulkWeatherService
            .refreshAllLocationsAsync(7);
    ...
    @Test
    void refreshAllLocationsAsync_WithLocations_RefreshesBothWeatherAndForecast() throws ExecutionException, InterruptedException {
        // Arrange
        List<LocationDto> locations = Arrays.asList(locationDto, locationDto);
        when(locationService.getAllLocations()).thenReturn(locations);
        when(weatherService.getCurrentWeatherByLocationId(anyLong(), eq(true)))
            .thenReturn(weatherDto);
    ...
    @Test
    void refreshAllLocationsAsync_WithPartialFailures_ReturnsPartialResult() throws ExecutionException, InterruptedException {
        // Arrange
