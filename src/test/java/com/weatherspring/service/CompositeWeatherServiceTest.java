 ...
 *
 * <p>Tests concurrent composite operations using virtual threads
 * and CompletableFuture with timeout handling.</p>
 */
...
class CompositeWeatherServiceTest {
    ...
    void setUp() {
        ...
        compositeWeatherService = new CompositeWeatherService(
            weatherService,
            forecastService,
            locationService,
            virtualExecutor
    ...
    void getWeatherAndForecast_WithValidLocation_ReturnsBothResults() {
        ...
        when(weatherService.getCurrentWeather(locationName, false))
            .thenReturn(weatherDto);
        when(forecastService.getForecast(locationName, 3, false))
            .thenReturn(forecasts);

        // Act
        WeatherWithForecast result = compositeWeatherService
            .getWeatherAndForecast(locationName, 3, false);
    ...
    void getWeatherAndForecast_WithSaveToDatabase_SavesBothResults() {
        ...
        when(weatherService.getCurrentWeather(locationName, true))
            .thenReturn(weatherDto);
        when(forecastService.getForecast(locationName, 7, true))
            .thenReturn(forecasts);

        // Act
        WeatherWithForecast result = compositeWeatherService
            .getWeatherAndForecast(locationName, 7, true);
    ...
    void getWeatherAndForecastByLocationId_WithValidLocationId_ReturnsBothResults() {
        ...
        when(weatherService.getCurrentWeatherByLocationId(locationId, false))
            .thenReturn(weatherDto);
        when(forecastService.getForecastByLocationId(locationId, 5, false))
            .thenReturn(forecasts);

        // Act
        WeatherWithForecast result = compositeWeatherService
            .getWeatherAndForecastByLocationId(locationId, 5, false);
    ...
    void getCompleteLocationInfo_WithValidLocationId_ReturnsAllThreeResults() {
        ...
        when(weatherService.getCurrentWeatherByLocationId(locationId, false))
            .thenReturn(weatherDto);
        when(forecastService.getForecastByLocationId(locationId, 3, false))
            .thenReturn(forecasts);

        // Act
        CompleteLocationInfo result = compositeWeatherService
            .getCompleteLocationInfo(locationId, 3, false);
    ...
    void getBulkWeather_WithMultipleLocations_ReturnAllWeatherData() {
        ...
        // Act
        List<WeatherDto> results = compositeWeatherService
            .getBulkWeather(locations, false);
