 ...
 *
 * <p>This component ensures that all required configuration values are set
 * before the application starts accepting requests. If validation fails,
 * the application will fail to start with a clear error message.</p>
 */
@Component
@Slf4j
...
public class ConfigurationValidator {
    ...
    public void validateConfiguration() {
        ...
        validateRequired("weather.api.base-url", weatherApiBaseUrl,
                "Weather API base URL is required.");
     ...
     *
     * <p>Performs multiple validations:</p>
     * <ul>
     *     <li>Checks if key is present and not blank</li>
     *     <li>Detects common placeholder values</li>
     *     <li>Validates minimum length (warns if < 20 characters)</li>
     * </ul>
     *
    ...
    private void validateApiKey() {
        ...
        if (weatherApiKey.equalsIgnoreCase("your-api-key-here") ||
            ...
            String message = "Weather API key is set to a placeholder value: " + weatherApiKey + ". " +
        ...
        if (weatherApiKey.length() < 20) {
            log.warn("Weather API key seems unusually short ({} characters). " +
                "Please verify it's a valid key.", weatherApiKey.length());
        }
    ...
    private void validateRequired(String propertyName, String value, String message) {
        if (value == null || value.isBlank()) {
            String errorMsg = String.format("Configuration validation failed for '%s': %s",
