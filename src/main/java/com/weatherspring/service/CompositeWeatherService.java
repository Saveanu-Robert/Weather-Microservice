 ...
 *
 * <p>This service demonstrates concurrent operations using CompletableFuture with
 * virtual thread executor (Java 21+ Project Loom) for massive scalability.</p>
 *
 * <p>Benefits of virtual threads:</p>
 * <ul>
 *     <li>Handles 10,000+ concurrent requests with minimal overhead</li>
 *     <li>No need for complex async programming models</li>
 *     <li>Simple blocking code that scales like async</li>
 *     <li>Perfect for I/O-bound operations like API calls</li>
 * </ul>
 *
...
public class CompositeWeatherService {
     ...
     */
    @Schema(description = "Composite result containing current weather and forecast data fetched concurrently")
     ...
     * <ul>
     *     <li>Both tasks run concurrently on separate virtual threads</li>
     *     <li>Waits for both to complete using allOf()</li>
     *     <li>Returns combined result</li>
     * </ul>
     *
     * <p><strong>No @Transactional:</strong> This method orchestrates parallel service calls,
     * each with their own transaction management. Adding @Transactional here would:
     * <ul>
     *     <li>Hold database connections open during blocking .join() calls</li>
     *     <li>Cause connection pool exhaustion under load</li>
     *     <li>Provide no benefit since no direct database operations occur here</li>
     * </ul>
     *
    ...
    public WeatherWithForecast getWeatherAndForecast(
        ...
        CompletableFuture<WeatherDto> weatherFuture = CompletableFuture.supplyAsync(() ->
                        weatherService.getCurrentWeather(locationName, saveToDatabase),
                virtualExecutor);

        CompletableFuture<List<ForecastDto>> forecastFuture = CompletableFuture.supplyAsync(() ->
        ...
        try {
            CompletableFuture.allOf(weatherFuture, forecastFuture)
            ...
            log.info("Successfully fetched weather and {}-day forecast for: {}",
                    forecastDays, locationName);
     ...
     * transactions. Blocking .join() calls should not hold database connections open.</p>
     *
     ...
     * @return composite result with weather and forecast
     */
    @Observed(name = "composite.weather.and.forecast.by.id", contextualName = "get-weather-and-forecast-by-id")
    public WeatherWithForecast getWeatherAndForecastByLocationId(
        ...
        try {
            ...
            log.info("Successfully fetched weather and {}-day forecast for location ID: {}",
                    forecastDays, locationId);
     ...
     */
    @Schema(description = "Complete location information including location details, current weather, and forecasts")
    public record CompleteLocationInfo(
            @Schema(description = "Location details (name, country, coordinates, etc.)")
            LocationDto location,

            @Schema(description = "Current weather information for this location")
            WeatherDto weather,
     ...
     *
     * <p>This demonstrates a 3-way concurrent fetch using virtual threads.</p>
     *
     * <p><strong>No @Transactional:</strong> Orchestrates parallel service calls with their own
     * transactions. Blocking .join() calls should not hold database connections open.</p>
     *
     ...
     */
    @Observed(name = "composite.complete.location.info", contextualName = "get-complete-location-info")
    public CompleteLocationInfo getCompleteLocationInfo(
        ...
        CompletableFuture<LocationDto> locationFuture = CompletableFuture.supplyAsync(() ->
     ...
     *
     * <p>Uses virtual threads to fetch weather for all locations in parallel.</p>
     *
     * <p><strong>No @Transactional:</strong> Orchestrates parallel service calls with their own
     * transactions. Blocking .join() calls should not hold database connections open.</p>
     *
    ...
    public List<WeatherDto> getBulkWeather(
        ...
        List<CompletableFuture<WeatherDto>> futures = locationNames.stream()
