...
public class ForecastService {
     ...
     * @throws org.springframework.dao.DataAccessException if database save fails (when saveToDatabase is true)
     */
    @Observed(name = "forecast.get", contextualName = "get-forecast")
    @Transactional(propagation = Propagation.REQUIRED)
    @Cacheable(value = "forecasts", key = "'forecast:byName:' + #locationName + ':' + #days", unless = "#result == null || #result.isEmpty()")
    public List<ForecastDto> getForecast(@NotBlank String locationName, @Min(1) @Max(14) int days, boolean saveToDatabase) {
        ...
        log.debug("Successfully fetched {}-day forecast for location: {} ({} forecast records)",
                days, locationName, result.size());
        return result;
    }
     ...
     * @throws com.weatherspring.exception.LocationNotFoundException if location with given ID does not exist
     * @throws com.weatherspring.exception.WeatherApiException if the external API call fails
     * @throws org.springframework.dao.DataAccessException if database save fails (when saveToDatabase is true)
     */
    @Observed(name = "forecast.get.by.location.id", contextualName = "get-forecast-by-location")
    @Transactional(propagation = Propagation.REQUIRED)
    @Cacheable(value = "forecasts", key = "'forecast:byId:' + #locationId + ':' + #days", unless = "#result == null || #result.isEmpty()")
    public List<ForecastDto> getForecastByLocationId(@NotNull Long locationId, @Min(1) @Max(14) int days, boolean saveToDatabase) {
        ...
        log.debug("Successfully fetched {}-day forecast for location ID: {} ({}) - {} forecast records",
                days, locationId, location.getName(), result.size());
        return result;
    }
    ...
    public List<ForecastDto> getStoredForecasts(@NotNull Long locationId) {
        ...
        List<ForecastDto> result = forecastRecordRepository.findByLocationId(locationId)
                .stream()
                .map(forecastMapper::toDto)
                .toList();

        log.debug("Successfully retrieved {} stored forecast records for location ID: {}", result.size(), locationId);
        return result;
    }
    ...
    public List<ForecastDto> getFutureForecasts(@NotNull Long locationId) {
        ...
        List<ForecastDto> result = forecastRecordRepository.findFutureForecastsByLocationId(locationId, today)
                .stream()
                .map(forecastMapper::toDto)
                .toList();

        log.debug("Successfully retrieved {} future forecast records for location ID: {} (after {})",
                result.size(), locationId, today);
        return result;
    }
     ...
     * @return list of forecast DTOs
     * @throws IllegalArgumentException if locationId, startDate, or endDate is null, or if startDate is after endDate
     * @throws com.weatherspring.exception.LocationNotFoundException if location with given ID does not exist
     * @throws org.springframework.dao.DataAccessException if database query fails
     */
    @Transactional(readOnly = true)
    ...
    public List<ForecastDto> getForecastsByDateRange(
        ...
        log.debug("Fetching forecasts for location ID {} from {} to {}",
                    locationId, startDate, endDate);

        locationService.getLocationEntityById(locationId);

        List<ForecastDto> result = forecastRecordRepository
                .findByLocationIdAndForecastDateBetween(locationId, startDate, endDate)
                .stream()
                .map(forecastMapper::toDto)
                .toList();

        log.debug("Successfully retrieved {} forecast records for location ID: {} in date range {} to {}",
     ...
     *
     * <p><strong>Optimized for bulk operations:</strong> Prevents N+1 queries by checking
     * for existing forecasts in a single database query before saving. This approach:</p>
     * <ul>
     *     <li>Extracts all forecast dates from records to be saved</li>
     *     <li>Queries existing records in bulk (single SELECT)</li>
     *     <li>Filters out duplicates using a Set lookup (O(1) per record)</li>
     *     <li>Saves only new records in batch (single INSERT batch)</li>
     * </ul>
     *
     * <p>This eliminates the N+1 problem that would occur if we saved individually
     * and caught exceptions for each duplicate.</p>
     *
    ...
    private void saveForecastRecords(ForecastApiResponse apiResponse) {
        ...
        List<LocalDate> forecastDates = forecastRecords.stream()
                .map(ForecastRecord::getForecastDate)
                .toList();

        // Bulk query to find existing records (single SELECT instead of N queries)
        List<ForecastRecord> existingRecords = forecastRecordRepository
                .findByLocationIdAndForecastDateIn(location.getId(), forecastDates);
        ...
        List<ForecastRecord> newRecords = forecastRecords.stream()
                .filter(record -> !existingDates.contains(record.getForecastDate()))
                .toList();

        if (!newRecords.isEmpty()) {
            forecastRecordRepository.saveAll(newRecords);
            log.info("Saved {} new forecast records for location: {} ({} duplicates skipped)",
                    newRecords.size(), location.getName(), forecastRecords.size() - newRecords.size());
        } else {
            log.info("No new forecast records to save for location: {} (all {} records already exist)",
                    location.getName(), forecastRecords.size());
        }
    }
     ...
     *
     * <p>This method runs in a separate transaction (REQUIRES_NEW) to ensure
     * cleanup operations don't interfere with other transactions.</p>
     *
