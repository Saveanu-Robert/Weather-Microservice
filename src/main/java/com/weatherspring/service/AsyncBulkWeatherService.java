 ...
 * allowing the caller to continue processing while data is being fetched.</p>
 *
 * <p>Benefits:</p>
 * <ul>
 *     <li>Non-blocking - caller can do other work while waiting</li>
 *     <li>Composable - can combine multiple async operations</li>
 *     <li>Virtual threads - no thread pool exhaustion</li>
 *     <li>Built-in error handling via CompletableFuture</li>
 * </ul>
 */
@Service
@Slf4j
...
public class AsyncBulkWeatherService {
    ...
    public AsyncBulkWeatherService(WeatherService weatherService,
        ...
        // Initialize counters
        this.weatherSuccessCounter = Counter.builder("async.bulk.weather.success")
                .description("Number of successful bulk weather fetches")
                .register(meterRegistry);
        this.weatherFailureCounter = Counter.builder("async.bulk.weather.failure")
                .description("Number of failed bulk weather fetches")
                .register(meterRegistry);
        this.forecastSuccessCounter = Counter.builder("async.bulk.forecast.success")
                .description("Number of successful bulk forecast fetches")
                .register(meterRegistry);
        this.forecastFailureCounter = Counter.builder("async.bulk.forecast.failure")
                .description("Number of failed bulk forecast fetches")
                .register(meterRegistry);
        this.updateSuccessCounter = Counter.builder("async.bulk.update.success")
                .description("Number of successful bulk weather updates")
                .register(meterRegistry);
        this.updateFailureCounter = Counter.builder("async.bulk.update.failure")
                .description("Number of failed bulk weather updates")
                .register(meterRegistry);

        // Initialize timers
        this.weatherTimer = Timer.builder("async.bulk.weather.duration")
                .description("Duration of bulk weather fetch operations")
                .register(meterRegistry);
        this.forecastTimer = Timer.builder("async.bulk.forecast.duration")
                .description("Duration of bulk forecast fetch operations")
                .register(meterRegistry);
        this.updateTimer = Timer.builder("async.bulk.update.duration")
     ...
     *
     * <p>Returns immediately with a CompletableFuture that will complete when all
     * weather data has been fetched.</p>
     *
    ...
    public CompletableFuture<List<WeatherDto>> fetchBulkWeatherAsync(
        ...
        return CompletableFuture.supplyAsync(() -> weatherTimer.record(() -> {
            log.info("Starting async bulk weather fetch for {} locations", locationNames.size());
            ...
            if (batchCount > 1) {
                log.info("Processing {} locations in {} batches of up to {} items",
                        locationNames.size(), batchCount, BatchingUtils.DEFAULT_BATCH_SIZE);
            }

            // Process each batch concurrently
            List<CompletableFuture<List<WeatherDto>>> batchFutures = batches.stream()
            ...
            List<WeatherDto> results = allBatches.thenApply(v -> batchFutures.stream()
     ...
     *
     * <p><strong>Failure Handling:</strong> Returns {@code null} for failed fetches
     * (exceptions caught in {@code exceptionally} handler). Nulls are filtered out
     * before returning, allowing metrics to distinguish failures from successful
     * empty results.</p>
     */
    private CompletableFuture<List<WeatherDto>> processBatchWeather(
            List<String> batch, boolean saveToDatabase) {

        List<CompletableFuture<WeatherDto>> futures = batch.stream()
    ...
    public CompletableFuture<List<List<ForecastDto>>> fetchBulkForecastAsync(
        ...
        return CompletableFuture.supplyAsync(() -> forecastTimer.record(() -> {
            log.info("Starting async bulk forecast fetch for {} locations, {} days",
                    locationNames.size(), days);
            ...
            // Process each batch concurrently
            List<CompletableFuture<List<List<ForecastDto>>>> batchFutures = batches.stream()
            ...
            List<List<ForecastDto>> results = allBatches.thenApply(v -> batchFutures.stream()
                    .map(CompletableFuture::join)
                    .flatMap(List::stream)
                    .toList()).join();

            // Record metrics
            // Count successes as the number of results returned (nulls filtered out at line 263)
     ...
     * <p><strong>Failure Handling:</strong> Returns {@code null} for failed fetches
     * (exceptions caught in {@code exceptionally} handler). Nulls are filtered out
     * before returning. An empty forecast list means successful fetch with no forecasts,
     * while {@code null} means the fetch itself failed.</p>
     */
    private CompletableFuture<List<List<ForecastDto>>> processBatchForecast(
            List<String> batch, int days, boolean saveToDatabase) {

        List<CompletableFuture<List<ForecastDto>>> futures = batch.stream()
                .map(name -> CompletableFuture.supplyAsync(() ->
     ...
     *
     * <p>Useful for batch jobs that need to refresh weather data for all tracked locations.</p>
     *
    ...
    public CompletableFuture<Integer> bulkUpdateWeatherAsync(
        ...
        return CompletableFuture.supplyAsync(() -> updateTimer.record(() -> {
            ...
            if (batchCount > 1) {
                log.info("Processing {} location IDs in {} batches of up to {} items",
                        locationIds.size(), batchCount, BatchingUtils.DEFAULT_BATCH_SIZE);
            }
            ...
            int totalSuccess = allBatches.thenApply(v -> batchFutures.stream()
                    .map(CompletableFuture::join)
            ...
            log.info("Bulk weather update completed: {}/{} successful",
                    totalSuccess, locationIds.size());
            return totalSuccess;
        }));
    }
     ...
     */
    private CompletableFuture<Integer> processBatchUpdate(
            List<Long> batch, boolean saveToDatabase) {

        List<CompletableFuture<WeatherDto>> futures = batch.stream()
                .map(id -> CompletableFuture.supplyAsync(() ->
                        weatherService.getCurrentWeatherByLocationId(id, saveToDatabase))
                        .exceptionally(ExceptionHandlers.warnWithMessageAndReturnNull(
        ...
        return allOf.thenApply(v -> (int) futures.stream()
                .map(CompletableFuture::join)
    ...
    public CompletableFuture<Integer> bulkRefreshForecastsAsync(
        ...
        return CompletableFuture.supplyAsync(() -> forecastTimer.record(() -> {
            log.info("Starting async bulk forecast refresh for {} locations, {} days",
                    locationIds.size(), days);
            ...
            log.info("Bulk forecast refresh completed: {}/{} successful",
                    totalSuccess, locationIds.size());
            return totalSuccess;
        }));
    }
     ...
     *
     * <p>Fetches both current weather and forecast for all locations in the system.
     * Returns a summary of the operation.</p>
     *
    ...
    public CompletableFuture<BulkOperationResult> refreshAllLocationsAsync(int forecastDays) {
        ...
        var locationIds = allLocations.stream()
                .map(loc -> loc.id())
                .toList();
        ...
        CompletableFuture<Integer> weatherFuture =
                bulkUpdateWeatherAsync(locationIds, true);

        CompletableFuture<Integer> forecastFuture =
                bulkRefreshForecastsAsync(locationIds, forecastDays, true);

        return weatherFuture.thenCombine(forecastFuture, (weatherSuccess, forecastSuccess) -> {
            ...
            log.info("Refresh all completed: {}/{} operations successful",
                    totalSuccess, totalOps);
