...
public class LocationService {
     ...
     *
     * <p><strong>Cache Strategy:</strong> Uses {@code allEntries = true} to evict the entire locations cache
     * because this operation affects:</p>
     * <ul>
     *     <li>{@code getAllLocations()} - the new location will appear in the full list</li>
     *     <li>All paginated queries - the new location may appear on any page depending on page size</li>
     *     <li>Cannot selectively evict paginated caches as page keys are dynamic ({@code page:X:Y})</li>
     * </ul>
     *
     * @param request the create location request
     * @return the created location DTO
     * @throws IllegalArgumentException if request is null or if location already exists with the same name and country
    ...
    public LocationDto createLocation(@Valid @NotNull CreateLocationRequest request) {
        ...
        log.debug("Successfully created location: {} (lat: {}, lon: {})",
     ...
     */
    @Transactional(readOnly = true)
    @Cacheable(value = "locations", key = "'location:page:' + #pageable.pageNumber + ':' + #pageable.pageSize")
    public Page<LocationDto> getAllLocations(@NotNull Pageable pageable) {
        log.debug("Fetching locations page {} with size {}", pageable.getPageNumber(), pageable.getPageSize());

        Page<LocationDto> result = locationRepository.findAll(pageable)
                .map(locationMapper::toDto);

        log.debug("Successfully retrieved {} locations (page {} of {})",
    ...
    public List<LocationDto> searchLocationsByName(@NotBlank String name) {
        ...
        List<LocationDto> result = locationRepository.findByNameContaining(name).stream()
                .map(locationMapper::toDto)
                .toList();
    ...
    public Page<LocationDto> searchLocationsByName(@NotBlank String name, @NotNull Pageable pageable) {
        log.debug("Searching locations by name '{}' - page {} with size {}",
                name, pageable.getPageNumber(), pageable.getPageSize());

        Page<LocationDto> result = locationRepository.findByNameContaining(name, pageable)
                .map(locationMapper::toDto);

        log.debug("Successfully found {} locations matching name '{}' (page {} of {})",
                result.getNumberOfElements(), name, result.getNumber() + 1, result.getTotalPages());
        return result;
    }
     ...
     *
     * <p>This method handles race conditions by using database-level unique constraints.
     * If concurrent creation attempts occur, the constraint violation is caught and
     * a retry query is performed.</p>
     *
     ...
     */
    @Transactional(propagation = Propagation.REQUIRED)
    public Location findOrCreateLocation(@NotNull String name, @NotNull String country,
        ...
        try {
            ...
            log.debug("Concurrent location creation detected for {} in {}, retrying query", name, country);

            // Retry the query - the location must exist now
            return locationRepository.findByNameAndCountry(name, country)
                    .orElseThrow(() -> new IllegalStateException(
                        "Location creation failed and retry query found nothing for: " + name + " in " + country));
        }
    }
     ...
     *
     * <p><strong>Cache Strategy:</strong> Uses {@code allEntries = true} to evict the entire locations cache
     * because updates to location name or coordinates affect:</p>
     * <ul>
     *     <li>The specific location cache entry (by ID)</li>
     *     <li>{@code getAllLocations()} - the updated location data</li>
     *     <li>All paginated queries - the updated location appears on its current page</li>
     *     <li>Search results - name changes affect {@code findByNameContaining()} results</li>
     * </ul>
     *
     * @param id the location ID
    ...
    @CacheEvictingOperation(cacheNames = "locations", allEntries = true)
    public LocationDto updateLocation(@NotNull Long id, @Valid @NotNull CreateLocationRequest request) {
        ...
        log.debug("Successfully updated location ID: {} - new values: {} (lat: {}, lon: {})",
     ...
     *
     * <p><strong>Cache Strategy:</strong> Uses {@code allEntries = true} to evict the entire locations cache
     * because deletion affects:</p>
     * <ul>
     *     <li>The specific location cache entry (by ID) - must be removed</li>
     *     <li>{@code getAllLocations()} - the deleted location no longer appears</li>
     *     <li>All paginated queries - page contents shift, potentially affecting multiple pages</li>
     *     <li>Search results - the deleted location no longer matches any searches</li>
     * </ul>
     *
     * @param id the location ID
     * @throws IllegalArgumentException if id is null
