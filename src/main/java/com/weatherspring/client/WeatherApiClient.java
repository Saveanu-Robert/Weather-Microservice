...
/**
 * Connects to WeatherAPI.com to fetch current weather and forecasts.
 * Uses circuit breaker and retry patterns for reliability.
 */
@Slf4j
@Component
public class WeatherApiClient {
        ...
        try {
            WeatherApiResponse response = restClient.get()
                    ...
                    .onStatus(HttpStatusCode::is4xxClientError, (request, clientResponse) -> {
                        log.error("Client error fetching weather for location {}: {}",
                                location, clientResponse.getStatusCode());
                    ...
                    .onStatus(HttpStatusCode::is5xxServerError, (request, serverResponse) -> {
                        log.error("Server error from WeatherAPI: {}",
    ...
    /**
     * Gets weather forecast for a location (1-14 days). Uses 2 retries instead of 3 since
     * forecasts have larger payloads and take longer.
     */
    ...
    public ForecastApiResponse getForecast(String location, int days) {
        ...
        if (days < ValidationConstants.FORECAST_DAYS_MIN || days > ValidationConstants.FORECAST_DAYS_MAX) {
        ...
        try {
            ForecastApiResponse response = restClient.get()
                    ...
                    .retrieve()
                    .onStatus(HttpStatusCode::is4xxClientError, (request, clientResponse) -> {
                        log.error("Client error fetching forecast for location {}: {}",
                                location, clientResponse.getStatusCode());
                        throw new WeatherApiException("Invalid location or API request: " + location);
                    })
                    .onStatus(HttpStatusCode::is5xxServerError, (request, serverResponse) -> {
    ...
    private WeatherApiResponse getCurrentWeatherFallback(String location, Throwable throwable) {
        log.error("Circuit breaker fallback triggered for getCurrentWeather. Location: {}, Error: {}",
    ...
    private ForecastApiResponse getForecastFallback(String location, int days, Throwable throwable) {
        log.error("Circuit breaker fallback triggered for getForecast. Location: {}, Days: {}, Error: {}",
                location, days, throwable.getMessage());
        throw new WeatherApiException(
                "Weather forecast service is currently unavailable. Please try again later. Location: " + location,
                throwable
        );
    }
}
