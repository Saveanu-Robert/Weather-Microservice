 ...
 *
 * <p>Provides CRUD operations and custom queries for weather forecast data.
 * Uses @EntityGraph and JOIN FETCH to prevent N+1 queries by eagerly fetching the location relationship.</p>
 */
@Repository
public interface ForecastRecordRepository extends JpaRepository<ForecastRecord, Long> {
     ...
     * Uses JOIN FETCH to eagerly load location, preventing N+1 queries.
     *
     * @param locationId the location ID
     * @return list of forecast records with locations eagerly loaded
     */
    @Query("SELECT f FROM ForecastRecord f JOIN FETCH f.location WHERE f.location.id = :locationId " +
           "ORDER BY f.forecastDate ASC")
    List<ForecastRecord> findByLocationId(@NotNull @Param("locationId") Long locationId);

    /**
     * Finds all forecast records for a specific location (paginated).
     * Uses @EntityGraph to eagerly load location, preventing N+1 queries.
     *
     * @param locationId the location ID
    ...
    @Query("SELECT f FROM ForecastRecord f WHERE f.location.id = :locationId ORDER BY f.forecastDate ASC")
    Page<ForecastRecord> findByLocationId(@NotNull @Param("locationId") Long locationId, @NotNull Pageable pageable);
    ...
    /**
     * Finds forecast records for a location within a date range (paginated).
     * Uses @EntityGraph to eagerly load location, preventing N+1 queries.
     *
     * @param locationId the location ID
     * @param startDate start of the date range
     ...
     * Uses @EntityGraph to eagerly fetch location in a single query.
     *
     * @param locationId the location ID
     ...
     */
    @EntityGraph(attributePaths = {"location"})
    Optional<ForecastRecord> findByLocationIdAndForecastDate(@NotNull Long locationId, @NotNull LocalDate forecastDate);
    ...
    /**
     * Finds all future forecasts for a location (paginated).
     * Uses @EntityGraph to eagerly load location, preventing N+1 queries.
     *
     * @param locationId the location ID
     * @param currentDate the current date
    ...
    /**
     * Finds existing forecast records for a location on multiple specific dates.
     * Uses JOIN FETCH to eagerly load location, preventing N+1 queries.
     *
     * <p>This method is essential for bulk operations to check which forecasts already exist
     * before attempting to save, avoiding unique constraint violations and N+1 queries.</p>
     *
     * @param locationId the location ID
