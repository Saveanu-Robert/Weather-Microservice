 ...
 *
 * <p>Provides CRUD operations and custom queries for historical weather data.
 * Uses @EntityGraph to prevent N+1 queries by eagerly fetching the location relationship.</p>
 */
@Repository
public interface WeatherRecordRepository extends JpaRepository<WeatherRecord, Long> {
     ...
     * Uses @EntityGraph to eagerly fetch the location in a single query, preventing N+1 problem.
     *
     ...
     */
    @Query("SELECT w FROM WeatherRecord w JOIN FETCH w.location WHERE w.location.id = :locationId " +
    ...
    /**
     * Finds weather records for a location within a date range (paginated).
     ...
     * Uses JOIN FETCH to eagerly load location in a single query.
     *
     * @param locationId the location ID
     ...
     *
     * <p>Note: Transaction management is handled by the calling service method
     * ({@code WeatherService.deleteOldWeatherRecords}).</p>
     *
