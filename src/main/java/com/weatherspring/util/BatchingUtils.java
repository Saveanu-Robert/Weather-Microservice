 ...
 *
 * <p>Provides methods to partition large collections into smaller batches for
 * processing. This is essential for bulk operations to prevent OutOfMemoryError
...
public final class BatchingUtils {
     ...
     *
     * <p>Set to 50 based on the following considerations:</p>
     * <ul>
     *     <li>Balance between API rate limiting (WeatherAPI.com allows 1M calls/month)</li>
     *     <li>Memory usage per batch: ~50 weather records ≈ 10-20KB</li>
     *     <li>Database batch insert optimal performance range (20-100 records)</li>
     *     <li>Virtual thread overhead acceptable for this size</li>
     * </ul>
     *
     * <p><strong>Tuning:</strong> Applications can override by calling
     * {@link #partition(List, int)} with a custom batch size. For heavy loads,
     * consider reducing to 20-30. For lighter loads with fewer API calls, increase to 80-100.</p>
     */
     ...
     *
     * <p>Set to 100 based on:</p>
     * <ul>
     *     <li>JVM heap analysis: 100 records ≈ 40KB max per batch</li>
     *     <li>Database connection pool limits (default: 10 connections)</li>
     *     <li>API timeout considerations (larger batches = longer processing time)</li>
     * </ul>
     *
     * <p><strong>Note:</strong> Exceeding this limit will throw {@link IllegalArgumentException}
     * via {@link #validateBatchSize(int)}. If you need larger batches, consider processing
     * in multiple stages or increasing JVM heap size.</p>
     */
     ...
     *
     * <p>The last batch may contain fewer elements than the batch size if the list
     * size is not evenly divisible by the batch size.</p>
     *
     * <p>Example:</p>
     * <pre>{@code
     ...
     *
     * <p>Uses {@link #DEFAULT_BATCH_SIZE} as the batch size.</p>
     *
     * @param <T> the type of elements in the list
     * @param list the list to partition
