 ...
 *
 * <p>This class encapsulates the common pattern of:</p>
 * <ul>
 *     <li>Partitioning input into batches</li>
 *     <li>Processing each batch concurrently</li>
...
public class AsyncBatchProcessor<T, R> {
    ...
    public AsyncBatchProcessor(MeterRegistry meterRegistry, String operationName) {
        ...
        // Initialize metrics
        this.timer = Timer.builder("async.batch." + operationName + ".duration")
                .description("Duration of " + operationName + " batch operations")
                .register(meterRegistry);

        this.successCounter = Counter.builder("async.batch." + operationName + ".success")
                .description("Number of successful " + operationName + " operations")
                .register(meterRegistry);

        this.failureCounter = Counter.builder("async.batch." + operationName + ".failure")
     ...
     */
    public CompletableFuture<List<R>> processAsync(List<T> items, Function<T, CompletableFuture<R>> processor) {
        return CompletableFuture.supplyAsync(() -> timer.record(() -> {
            ...
            if (batchCount > 1) {
                log.info("Processing {} items in {} batches of up to {} items",
                        items.size(), batchCount, BatchingUtils.DEFAULT_BATCH_SIZE);
            }
            ...
            // Aggregate results
            List<R> results = allBatches.thenApply(v -> batchFutures.stream()
            ...
            log.info("Async {} completed: {}/{} successful", operationName, successCount, items.size());
            return results;
        }));
    }
     ...
     */
    private CompletableFuture<List<R>> processBatch(List<T> batch, Function<T, CompletableFuture<R>> processor) {
     ...
     *
     * <p>Useful when you don't need the actual results, just success count.</p>
     *
     * @param items the items to process
     * @param processor function that processes a single item
     * @return CompletableFuture containing count of successful operations
     */
    public CompletableFuture<Integer> processForCount(List<T> items, Function<T, CompletableFuture<R>> processor) {
        return processAsync(items, processor)
                .thenApply(List::size);
    }
}
